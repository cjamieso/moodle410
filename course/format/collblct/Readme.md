...
then please do spread the word to other educators.  The main page for the format is https://moodle.org/plugins/format_collblct.
...
===========================
The primary source for downloading this branch of the format is https://moodle.org/plugins/view.php?plugin=format_collblct
with 'Select Moodle version:' set at 'Moodle 4.1'.

The secondary source is a tagged version with the V401 prefix on https://github.com/gjb2048/moodle-format_collblct/tags

If you download from the development area - https://github.com/gjb2048/moodle-format_collblct/ - consider that
...
https://github.com/gjb2048/moodle-format_collblct and doing a 'Pull Request' so that the rest of the
...
============
 ...
 6. Copy 'collblct' to '/course/format/' if you have not already done so.
...
4. In '/course/format/' move old 'collblct' directory to a backup folder outside of Moodle.
...
2. In '/course/format/' move old 'collblct' directory to a backup folder outside of Moodle.
3. Do not copy in the new version of 'collblct' yet!  As this will cause the upgrade to fail.
4. Upgrade to Moodle 2.2 first - http://docs.moodle.org/22/en/Upgrading_to_Moodle_2.2.
5. After you have installed Moodle 2.2, now upgrade to Moodle 2.4 with this new collblct -
   http://docs.moodle.org/24/en/Upgrading_to_Moodle_2.4 - but before initiating the upgrade you can copy the
   new (i.e. this) 'collblct' folder to '/course/format'.
6. Now follow 'Upgrading from Moodle 2.2' below please.
INFO: Having no 'collblct' folder in '/course/format' is fine as the courses that use it are not accessed and
...
1.    Put Moodle in 'Maintenance Mode' so that there are no users using it bar you as the administrator - if you have
      not already done so.
2.    In '/course/format/' move old 'collblct' directory to a backup folder outside of Moodle - if you have not
      already done so.
3.    Copy this new 'collblct' folder to '/course/format/'.
4.    Upgrade to Moodle 2.4 by being logged in as 'admin' and clicking on 'Home'.  If you have previously upgraded but
      'collblct' was an old version and the upgrade failed, this should still work.
5.    Follow installation instructions above.
...
1.    In your database:
2.1   Rename the table 'format_collblct_layout' to 'format_collblct_settings'.
2.2   With the table 'format_collblct_settings' change all integer types to signed if using a MySQL database.
2.3   If the table 'format_collblct_settings' does not exist, then create it and add the following fields 
      in this order:
...
2.3.4 'layoutstructure' of type 'TINYINT(1)' type, not null, no auto increment, no zero fill with a default value
      of '1'.
2.4   With the table 'format_collblct_settings' append three new fields of 'VARCHAR(6)' type, not null, called
      'tgfgcolour', 'tgbgcolour' and 'tgbghvrcolour' in that order with the default values of '000000', 'e2e2f2'
      and 'eeeeff' respectively.
2.5   With the table 'format_collblct_settings' append a new field 'layoutcolumns' after the 'layoutstructure' field
...
2.5.1 'layoutcolumns' of type 'TINYINT(1)' type, not null, no auto increment, no zero fill with a default value
      of '1'.
2.6   Drop the table 'format_collblct_cookie_cnsnt'.
...
3. In '/course/format/' remove the folder 'collblct'.
4. In the database, remove the table 'format_collblct_settings' along with the entry for 'format_collblct'
   ('plugin' attribute) in the table 'config_plugins'.  If using the default prefix this will be
   'mdl_format_collblct_settings' and 'mdl_config_plugins' respectively.
...
3.  Sometimes when restoring a course, it is accessed for the first time and a toggle is clicked a 'Error updating user
    preference 'collblct_toggle_x'' (where 'x' is the course id as shown in the URL 'id=x') can occur.  I'm not completely sure
    ...
    if you encounter it.
4.  If you get HTTP 403 errors on the browsers console for the 'setcollblctpref.php' then check that the permissions within the
    'collblct' folder are 755 for folders and 644 for files.  Ref: https://moodle.org/mod/forum/discuss.php?d=329620.
