...
use \format_collblct\privacy\provider;

/**
 * Privacy unit tests for the Collapsed Topics course format.
 * @group format_collblct
 */
class format_collblct_privacy_provider_test extends \core_privacy\tests\provider_testcase {
    ...
    protected function set_up() {
        ...
        $this->course = $this->getDataGenerator()->create_course(array('format' => 'collblct', 'numsections' => $this->numsections),
    ...
    public function test_get_metadata() {
        $items = new collection('format_collblct');
    ...
    public function test_export_user_preferences() {
        $togglelib = new \format_collblct\togglelib;

        $this->set_up();
        $this->setAdminUser();

        set_user_preference('collblct_toggle_'.$this->course->id, 'FAB');
        ...
        $prefs = (array) $writer->get_user_preferences('format_collblct');

        $this->assertCount(1, $prefs);

        $toggle = $prefs['collblct_toggle_'.$this->course->id];
        $this->assertEquals('FAB', $toggle->value);

        $description = get_string('privacy:request:preference:toggle', 'format_collblct', (object) [
            'name' => $this->course->id,
            'value' => 'FAB',
