 ...
 * @group format_collblct
 */
class format_collblct_courseformatrenderer_test extends advanced_testcase {
    ...
    protected function init($numsections = 1, $layoutcolumnorientation = 2, $toggleallenabled = 2, $viewsinglesectionenabled = 2) {
        ...
        global $DB, $PAGE;
        $this->outputus = $PAGE->get_renderer('format_collblct');
        // Ref: https://docs.moodle.org/dev/Writing_PHPUnit_tests.
        $nosections = (empty($numsections)) ? true : false;
        if ($nosections) {
            $numsections = 1;
        }
        $this->course = $this->getDataGenerator()->create_course(array('format' => 'collblct', 'numsections' => $numsections),
    ...
    public function test_section_summary() {
        ...
        $thevalue = self::call_method($this->outputus, 'render_from_template', array('format_collblct/sectionsummary',
            $sectionsummarycontext));

        $this->assertEquals($thevalue, $theclass);
    }

    public function test_collblct_section() {
        $this->init();
        set_user_preference('collblct_toggle_'.$this->course->id, 'Z');
        set_config('defaultuserpreference', 0, 'format_collblct');
        set_config('defaulttogglepersistence', 1, 'format_collblct');
        ...
        $onsectionpage = false;
        $sectionreturn = null;
        $theclass = self::call_method($this->outputus, 'collblct_section',
        ...
        $thevalue = self::call_method($this->outputus, 'render_from_template', array('format_collblct/section', $sectioncontext));
        $this->assertEquals($thevalue, $theclass);
    ...
    public function test_section_hidden() {
        ...
        $thevalue = self::call_method($this->outputus, 'render_from_template', array('format_collblct/sectionhidden',
    ...
    public function test_stealth_section() {
        ...
        $thevalue = self::call_method($this->outputus, 'render_from_template', array('format_collblct/stealthsection',
    ...
    public function test_single_section_page() {
        ...
        $singlesectioncontext = array(
            ...
            'sectionzero' => self::call_method($this->outputus, 'collblct_section', array($sectionzero, $course, true, $displaysection, array('sr' => $displaysection))),
            'thissection' => self::call_method($this->outputus, 'collblct_section', array($thissection, $course, true, $displaysection, array('sr' => $displaysection)))
        );
        ...
        $theoutput = self::call_method($this->outputus, 'render_from_template', array('format_collblct/singlesection', $singlesectioncontext));
    ...
    */
    ...
    public function test_multiple_section_page_horizontal() {
        ...
        $this->init();
        set_user_preference('collblct_toggle_'.$this->course->id, null);
        set_config('defaultuserpreference', 0, 'format_collblct');
        set_config('defaulttogglepersistence', 1, 'format_collblct');
        ...
        $theoutput = file_get_contents($CFG->dirroot.'/course/format/collblct/tests/phpu_data/test_multiple_section_page_css.txt');
        $theoutput .= '<ul class="ctopics">';
        $theoutput .= self::call_method($this->outputus, 'collblct_section', array($section0, $this->course, false, 0));
        $theoutput .= '</ul><ul class="ctopics topics row">';
        $theoutput .= self::call_method($this->outputus, 'collblct_section', array($section1, $this->course, false));
        $theoutput .= '</ul>';
    ...
    public function test_toggle_all() {
        ...
        $thevalue = self::call_method($this->outputus, 'render_from_template', array('format_collblct/toggleall',
            $toggleallcontext));
    ...
    public function test_display_instructions() {
        ...
        $thevalue = self::call_method($this->outputus, 'render_from_template', array('format_collblct/displayinstructions',
