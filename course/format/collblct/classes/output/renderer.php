...
namespace format_collblct\output;
 ...
 * HOOK
 *<------------------------------------------------------------------------*/
require_once($CFG->dirroot.'/course/format/collblct/collapsed_label_hooks.php');
/*-------------------------------------------------------------------------->
 * END HOOK
 *<------------------------------------------------------------------------*/
...
class renderer extends section_renderer {
    ...
    public function __construct(\moodle_page $page, $target) {
        ...
        $this->togglelib = new \format_collblct\togglelib();
        ...
        /* Since format_collblct_renderer::section_edit_control_items() only displays the 'Set current section' control when editing
        ...
        $this->tconesectioniconfont = get_config('format_collblct', 'defaultonesectioniconfont');
        $this->tctoggleiconsize = get_config('format_collblct', 'defaulttoggleiconsize');
        $this->formatresponsive = get_config('format_collblct', 'formatresponsive');
    ...
    public function course_section_updated(
        ...
        $output = $this->collblct_section($section, $format->get_course(), false);
        ...
        if ($section->section != 0) {
            if ($this->userisediting) {
                ...
                if (($this->tcsettings['layoutstructure'] == 1) || ($this->tcsettings['layoutstructure'] == 4)) {
                    $topictext = get_string('setlayoutstructuretopic', 'format_collblct');
                } else if (($this->tcsettings['layoutstructure'] == 2) || ($this->tcsettings['layoutstructure'] == 3)) {
                    $topictext = get_string('setlayoutstructureweek', 'format_collblct');
                } else {
    ...
    protected function one_section_icon($title) {
        if (empty($this->tconesectioniconfont)) {
            return $this->output->pix_icon('one_section', $title, 'format_collblct');
        } else {
            $osicontext = array(
                'osifc' => $this->tconesectioniconfont,
                'osift' => $title
            );
            return $this->render_from_template('format_collblct/onesectioniconfont', $osicontext);
        }
    }
    ...
    protected function section_summary($section, $course, $mods) {
        $title = $this->courseformat->get_collblct_section_name($course, $section, false);
        ...
        return $this->render_from_template('format_collblct/sectionsummary', $sectionsummarycontext);
    }
     ...
     * @return string HTML to output.
     */
    protected function collblct_section($section, $course, $onsectionpage, $sectionreturn = null) {
        ...
        if (($onsectionpage == false) && ($section->section != 0)) {
            ...
            if ($this->userisediting) {
                ...
                $title = $this->courseformat->get_collblct_section_name($course, $section, true);
            }
            ...
            if ($this->userisediting) {
                // CONTRIB-7434.
                $sectioncontext['usereditingtitle'] = $this->courseformat->get_collblct_section_name($course, $section, false);
            }
        } else {
    ...
    protected function toggle_icon_set(&$sectioncontext) {
        if ($this->tcsettings['toggleiconset'] == 'tif') {
            ...
            $sectioncontext['tif'] = $this->render_from_template('format_collblct/tif', $tifcontext);
        } else {
    ...
    protected function stealth_section($section, $course) {
        ...
        return $this->render_from_template('format_collblct/stealthsection', $stealthsectioncontext);
    }
    ...
    protected function section_hidden($section, $courseorid = null) {
        ...
        return $this->render_from_template('format_collblct/sectionhidden', $sectionhiddencontext);
    }

    /**
     * Output the html for a single section page.
     *
     * @param stdClass $course The course from the format_collblct class.
    ...
    public function single_section_page($displaysection) {
        ...
        $maincoursepage = get_string('maincoursepage', 'format_collblct');
        ...
        $singlesectioncontext = array(
            ...
            'thissection' => $this->collblct_section($thissection, $course, true, $displaysection)
        );

        $sectionzero = $modinfo->get_section_info(0);
        if ($sectionzero->summary or !empty($modinfo->sections[0]) or $this->page->user_is_editing()) {
            $singlesectioncontext['sectionzero'] = $this->collblct_section($sectionzero, $course, true, $displaysection);
        }
    ...
    public function multiple_section_page() {
        ...
        if ($thissection->summary or ! empty($modinfo->sections[0]) or $this->userisediting) {
            $content .= $this->collblct_section($thissection, $course, false);
        }
        ...
        if ($coursenumsections > 0) {
            ...
            if (($this->tcsettings['layoutcolumns'] > 1) && ($this->mobiletheme === false)) {
                if ($this->tcsettings['layoutcolumns'] > 4) {
                    ...
                    // Update....
                    $this->courseformat->update_collblct_columns_setting($this->tcsettings['layoutcolumns']);
                }
            ...
            foreach ($sectiondisplayarray as $thissection) {
                ...
                if (!empty($thissection->ishidden)) {
                    ...
                    $sectionoutput .= $this->collblct_section($thissection, $course, false);
        ...
        $this->page->requires->js_init_call('M.format_collblct.init', array(
            $course->id,
            $toggles,
            $coursenumsections,
        ...
        set_user_preference('collblct_toggle_'.$course->id, $toggles);
    ...
    protected function toggle_all($toggledsections) {
        ...
        $toggleallcontext = array(
            'rtl' => $this->rtl,
            'sctcloseall' => get_string('sctcloseall', 'format_collblct', $sct),
            'sctopenall' => get_string('sctopenall', 'format_collblct', $sct),
        ...
        return $this->render_from_template('format_collblct/toggleall', $toggleallcontext);
    }
    ...
    protected function display_instructions() {
        ...
        return $this->render_from_template('format_collblct/displayinstructions', $displayinstructionscontext);
    }
    ...
    protected function course_styles() {
        ...
        $coursestylescontext['togglebackground'] = \format_collblct\toolbox::hex2rgba(
            $this->tcsettings['togglebackgroundcolour'], $this->tcsettings['togglebackgroundopacity']);
        $coursestylescontext['toggleforegroundcolour'] = \format_collblct\toolbox::hex2rgba(
        ...
        $coursestylescontext['toggleforegroundhovercolour'] = \format_collblct\toolbox::hex2rgba(
            $this->tcsettings['toggleforegroundhovercolour'], $this->tcsettings['toggleforegroundhoveropacity']);
        $coursestylescontext['togglebackgroundhovercolour'] = \format_collblct\toolbox::hex2rgba(
            $this->tcsettings['togglebackgroundhovercolour'], $this->tcsettings['togglebackgroundhoveropacity']);

        $collblctsidewidth = get_string('collblctsidewidthlang', 'format_collblct');
        $collblctsidewidthdelim = strpos($collblctsidewidth, '-');
        $collblctsidewidthlang = strcmp(substr($collblctsidewidth, 0, $collblctsidewidthdelim), current_language());
        $collblctsidewidthval = substr($collblctsidewidth, $collblctsidewidthdelim + 1);
        // Dynamically changing widths with language.
        if ((!$this->userisediting) && (($this->mobiletheme == false) && ($this->tablettheme == false)) && ($collblctsidewidthlang == 0)) {
            $coursestylescontext['collblctsidewidthval'] = $collblctsidewidthval;
        } else if ($this->userisediting) {
            $coursestylescontext['collblctsidewidthval'] = '40px';
        }

        // Make room for editing icons.
        if ((!$this->userisediting) && ($collblctsidewidthlang == 0)) {
            $coursestylescontext['collblctsidewidthvalicons'] = $collblctsidewidthval;
        }
        ...
        $coursestylescontext['tcborderradiustl'] = clean_param(get_config('format_collblct', 'defaulttoggleborderradiustl'), PARAM_TEXT);
        $coursestylescontext['tcborderradiustr'] = clean_param(get_config('format_collblct', 'defaulttoggleborderradiustr'), PARAM_TEXT);
        $coursestylescontext['tcborderradiusbr'] = clean_param(get_config('format_collblct', 'defaulttoggleborderradiusbr'), PARAM_TEXT);
        $coursestylescontext['tcborderradiusbl'] = clean_param(get_config('format_collblct', 'defaulttoggleborderradiusbl'), PARAM_TEXT);

        return $this->render_from_template('format_collblct/coursestyles', $coursestylescontext);
    }

    protected function set_user_preferences() {
        $this->defaultuserpreference = clean_param(get_config('format_collblct', 'defaultuserpreference'), PARAM_INT);
        $this->defaulttogglepersistence = clean_param(get_config('format_collblct', 'defaulttogglepersistence'), PARAM_INT);

        if ($this->defaulttogglepersistence == 1) {
            user_preference_allow_ajax_update('collblct_toggle_' . $this->course->id, PARAM_RAW);
            $userpreference = get_user_preferences('collblct_toggle_' . $this->course->id);
        } else {
            $userpreference = null;
        }
