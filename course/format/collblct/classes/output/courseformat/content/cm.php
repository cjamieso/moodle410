...
namespace format_collblct\output\courseformat\content;
...
class cm extends cm_base {
    ...
    public function export_for_template(\renderer_base $output): \stdClass {
        ...
        if (\format_collblct\activity::activitymetaenabled()) {
            ...
            if (\format_collblct\activity::activitymetaused($courseformat)) {
                $courseid = $this->mod->course;
                if (\format_collblct\activity::maxstudentsnotexceeded($courseid)) {
                    $settingname = 'coursesectionactivityfurtherinformation'.$this->mod->modname;
                    $setting = get_config('format_collblct', $settingname);
    ...
    protected function course_section_cm_get_meta(\cm_info $mod) {
        ...
        $meta = \format_collblct\activity::module_meta($mod);
        if ($meta == null) {
        ...
        if ($meta->isteacher) {
            ...
            if (!$meta->submissionnotrequired) {
                ...
                if ($meta->numparticipants != 0) {
                    $engagementmeta[] = get_string('xofy'.$meta->submitstrkey, 'format_collblct',
                        (object) array(
                    ...
                    $engagementmeta[] = get_string('x'.$meta->submitstrkey, 'format_collblct',
                        (object) array(
                            'completed' => $meta->numsubmissions
                        )
                    );
            ...
            if ($meta->numrequiregrading) {
                $engagementmeta[] = get_string('xungraded', 'format_collblct', $meta->numrequiregrading);
            }
            if (!empty($engagementmeta)) {
                ...
                $content = $OUTPUT->render_from_template('format_collblct/sectioncmmeta', $sectioncmmetacontext);
            }
        } else {
            // Feedback meta.
            if (!empty($meta->grade)) {
                ...
                $sectioncmmetacontext = array(
                    ...
                    'linktext' => get_string('feedbackavailable', 'format_collblct'),
                    'linkurl' => $url,
                    'type' => 'feedback'
                );
                $content = $OUTPUT->render_from_template('format_collblct/sectioncmmeta', $sectioncmmetacontext);
