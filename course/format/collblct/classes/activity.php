...
class activity {
    ...
    public static function module_meta(cm_info $mod) {
        ...
        if (method_exists('format_collblct\\activity', $methodname)) {
            $meta = call_user_func('format_collblct\\activity::' . $methodname, $mod);
        ...
        if ((has_capability('mod/assign:grade', $mod->context)) ||
            ...
            if ($mod->modname === 'assign') {
                ...
                if (!empty($meta->numparticipants)) {
                    ...
                    if (method_exists('format_collblct\\activity', $methodnsubmissions)) {
                        $meta->numsubmissions = call_user_func('format_collblct\\activity::'.
                            $methodnsubmissions, $courseid, $mod);
                    }
                    if (method_exists('format_collblct\\activity', $methodnumungraded)) {
                        $meta->numrequiregrading = call_user_func('format_collblct\\activity::'.
    ...
    protected static function forum_num_submissions($courseid, $mod) {
        ...
        $studentscache = \cache::make('format_collblct', 'activitystudentscache');
    ...
    protected static function course_participant_count($courseid, $mod) {
        ...
        // New users?
        $usercreatedcache = \cache::make('format_collblct', 'activityusercreatedcache');
        ...
        if (!empty($createdusers)) {
            ...
            $studentrolescache = \cache::make('format_collblct', 'activitystudentrolescache');
        ...
        if (is_array($students)) {
            // We have students!
            $modulecountcache = \cache::make('format_collblct', 'activitymodulecountcache');
    ...
    public static function maxstudentsnotexceeded($courseid, $extrainfo = false) {
        $notexceeded = true;
        $maxstudents = get_config('format_collblct', 'courseadditionalmoddatamaxstudents');
        $studentcount = 0;
    ...
    */
    ...
    private static function lockcaches($courseid) {
        $lockfactory = \core\lock\lock_config::get_lock_factory('format_collblct');
        if ($lock = $lockfactory->get_lock('courseid'.$courseid, 5)) {
            return $lock;
        }
        throw new \moodle_exception('cannotgetactivitycacheslock', 'format_collblct', '',
            get_string('cannotgetactivitycacheslock', 'format_collblct', $courseid));
    }
    ...
    public static function activitymetaenabled() {
        return (get_config('format_collblct', 'enableadditionalmoddata') == 2);
    }
