...
class admin_setting_information extends \admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        if (!empty($formats['collblct'])) {
            $plugininfo = $formats['collblct'];
        } else {
            $plugininfo = \core_plugin_manager::instance()->get_plugin_info('format_collblct');
            $plugininfo->version = $plugininfo->versiondisk;
        }

        $context['versioninfo'] = get_string('versioninfo', 'format_collblct',
            array(
                'moodle' => $CFG->release,
        ...
        if (!empty($plugininfo->maturity)) {
            switch ($plugininfo->maturity) {
                case MATURITY_ALPHA:
                    $context['maturity'] = get_string('versionalpha', 'format_collblct');
                    $context['maturityalert'] = 'danger';
                break;
                case MATURITY_BETA:
                    $context['maturity'] = get_string('versionbeta', 'format_collblct');
                    $context['maturityalert'] = 'danger';
                break;
                case MATURITY_RC:
                    $context['maturity'] = get_string('versionrc', 'format_collblct');
                    $context['maturityalert'] = 'warning';
                break;
                case MATURITY_STABLE:
                    $context['maturity'] = get_string('versionstable', 'format_collblct');
        ...
        if ($CFG->branch != $this->mbranch) {
            ...
            $context['versioncheck'] .= '<a href="https://moodle.org/plugins/format_collblct" target="_blank">Moodle.org</a>.  ';
        ...
        return $OUTPUT->render_from_template('format_collblct/ct_admin_setting_information', $context);
    }
}
