...
class togglelib {
     ...
     */
    public static function required_collblct_param($parname) {
        if (empty($parname)) {
            throw new coding_exception('required_collblct_param() requires $parname to be specified');
        }
        $param = required_param($parname, PARAM_RAW);

        return self::clean_collblct_param($param);
    }

    /**
     * Used by required_collblct_param to clean the toggle parameter.
     *
     * @param string $param the variable we are cleaning
     * @return mixed
     * @throws coding_exception
     */
    public static function clean_collblct_param($param) {
        if (is_array($param)) {
            throw new coding_exception('clean_collblct_param() can not process arrays.');
        } else if (is_object($param)) {
            if (method_exists($param, '__toString')) {
                $param = $param->__toString();
            } else {
                throw new coding_exception('clean_collblct_param() can not process objects.');
            }
        }
