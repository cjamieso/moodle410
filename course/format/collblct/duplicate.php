...
$PAGE->set_url('/course/format/collblct/duplicate.php', array('courseid' => $courseid, 'sectionno' => $sectionno));
...
if (!empty($sectioninfo)) {
    $pbar = new progress_bar('collblct_duplicate_bar', 500, true);
    $pbar->update_full(1, get_string('duplicating', 'format_collblct'));

    $lastsectionnum = $DB->get_field('course_sections', 'MAX(section)', array('course' => $courseid), MUST_EXIST);

    $numnewsection = $lastsectionnum + 1;

    $pbar->update_full(5, get_string('creatingsection', 'format_collblct'));
    ...
    $pbar->update_full(10, get_string('rebuildcoursecache', 'format_collblct'));
    ...
    if (is_object($modinfo) && isset($modinfo->sections[$sectionno])) {
        ...
        if (is_array($sectionmods)) {
            ...
            $k = 0;
            $pbar->update_full(40, get_string('progresscounter', 'format_collblct', $dataprogress));
            foreach ($sectionmods as $modnumber) {
                ...
                $pbar->update_full($percent, get_string('progresscounter', 'format_collblct', $dataprogress));
            }
        }
    } else {
        $pbar->update_full(100, get_string('progressfull', 'format_collblct'));
    }

    $sectiontogo = $numnewsection;
} else {
    $sectiontogo = $sectionno;
    echo get_string('errornosectioninfo', 'format_collblct');
