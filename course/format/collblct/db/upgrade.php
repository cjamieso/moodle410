...
function xmldb_format_collblct_upgrade($oldversion = 0) {
    ...
    if ($result && $oldversion < 2012070300) {
        ...
        $table = new xmldb_table('format_collblct_settings');   // Use the new table.
            ...
            foreach ($records as $record) {
                ...
                if ($DB->record_exists('course', array('id' => $record->courseid))) {
                    ...
                    if ($courseformat->get_format() == 'collblct') {
                        $courseformat->restore_collblct_setting($record->courseid, $record->layoutelement, $record->layoutstructure,
                                                               $record->layoutcolumns, $record->tgfgcolour, $record->tgbgcolour,
                                                               $record->tgbghvrcolour); // In '/course/format/collblct/lib.php'.
                    }
                }
            }
    ...
    if ($oldversion < 2017110301) {
        ...
        (SELECT id FROM {course} WHERE format=?)", ['collblct']);

        upgrade_plugin_savepoint(true, 2017110301, 'format', 'collblct');
    }

    if ($oldversion < 2020110902) {
        // Change in default names.
        $value = get_config('format_collblct', 'defaulttgfgcolour');
        set_config('defaulttoggleforegroundcolour', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgfgopacity');
        set_config('defaulttoggleforegroundopacity', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgfghvrcolour');
        set_config('defaulttoggleforegroundhovercolour', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgfghvropacity');
        set_config('defaulttoggleforegroundhoveropacity', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgbgcolour');
        set_config('defaulttogglebackgroundcolour', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgbgopacity');
        set_config('defaulttogglebackgroundopacity', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgbghvrcolour');
        set_config('defaulttogglebackgroundhovercolour', $value, 'format_collblct');

        $value = get_config('format_collblct', 'defaulttgbghvropacity');
        set_config('defaulttogglebackgroundhoveropacity', $value, 'format_collblct');

        upgrade_plugin_savepoint(true, 2020110902, 'format', 'collblct');
    }
