...
class format_collblct extends core_courseformat\base {
            ...
            foreach ($this->settings as $settingname => $settingvalue) {
                if (isset($settingvalue)) {
                    ...
                    if ((($settingvtype == 'string') && ($settingvalue === '-')) ||
                        ...
                        $this->settings[$settingname] = get_config('format_collblct', 'default'.$settingname);
                    }
                }
            }
        }
    ...
    /**
     * Returns the information about the ajax support. collblct uses ajax.
     *
     * @return stdClass
     */
    ...
    public function get_section_name($section) {
        ...
        return $this->get_collblct_section_name($course, $section, false);
    }
     ...
     * @return string The section name.
     */
    public function get_collblct_section_name($course, $section, $additional) {
        ...
        if ((string) $thesection->name !== '') {
            ...
            $o = get_string('section0name', 'format_collblct');
        } else {
            if (($tcsettings['layoutstructure'] == 1) || ($tcsettings['layoutstructure'] == 4)) {
                $o = get_string('sectionname', 'format_collblct') . ' ' . $thesection->section;
            } else {
                $o .= $this->get_section_dates($section, $course, $tcsettings);
            }
        }
        ...
        if (($additional == true) && ($thesection->section != 0)) {
            switch ($tcsettings['layoutelement']) {
                ...
                case 4:
                    // The word 'Toggle'.
                    $o .= '<div class="cttoggle"> - ' .get_string('collblcttoggle', 'format_collblct') . '</div>';
                    break;
            }
        }
    ...
    public function get_section_dates($section, $course = null, $tcsettings = null) {
        ...
        if ($tcsettings['layoutstructure'] == 5) {
            $day = $this->format_collblct_get_section_day($section, $course);

            $weekday = userdate($day, $dateformat);
            $o = $weekday;
        } else {
            $dates = $this->format_collblct_get_section_dates($section, $course);
    ...
    public function ajax_section_move() {
        ...
        $formatrenderer = $PAGE->get_renderer('format_collblct');
    ...
    public function get_default_blocks() {
        ...
        $blocklist = explode(',' , get_config('format_collblct' , 'defaultdisplayblocks'));
        if (empty($blocklist[0])) {
            return array();
        }
        if (get_config('format_collblct' , 'defaultdisplayblocksloc') == 2) {
            $bpr = $blocklist;
            $bpl = array();
        } else {
            $bpr = array();
    ...
    public function section_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($sectionformatoptions['donotshowdate']['label'])) {
            $sectionformatoptionsedit = array(
                'donotshowdate' => array(
                    'label' => new lang_string('donotshowdate', 'format_collblct'),
                    'help' => 'donotshowdate',
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            ...
            if (\format_collblct\activity::activitymetaenabled()) {
                $engagementactivities = array('assign', 'quiz', 'choice', 'feedback', 'forum', 'lesson', 'data');
                foreach ($engagementactivities as $plugintype) {
                    if (get_config('format_collblct', 'coursesectionactivityfurtherinformation'.$plugintype) == 2) {
                        switch ($plugintype) {
                            case 'assign':
                            ...
                            case 'choice' || 'feedback' || 'forum' || 'lesson':
                                ...
                                break;
                            default:
                                coding_exception('Try to process invalid plugin', 'Check the plugintypes that are supported by format_collblct');
                        }
                    }
                }
            }
        ...
        if ($foreditform && !isset($courseformatoptions['displayinstructions']['label'])) {
            ...
            $defaulttgfgcolour = get_config('format_collblct', 'defaulttoggleforegroundcolour');
            if ($defaulttgfgcolour[0] == '#') {
                $defaulttgfgcolour = substr($defaulttgfgcolour, 1);
            }
            $defaulttgfghvrcolour = get_config('format_collblct', 'defaulttoggleforegroundhovercolour');
            if ($defaulttgfghvrcolour[0] == '#') {
                $defaulttgfghvrcolour = substr($defaulttgfghvrcolour, 1);
            }
            $defaulttgbgcolour = get_config('format_collblct', 'defaulttogglebackgroundcolour');
            if ($defaulttgbgcolour[0] == '#') {
                $defaulttgbgcolour = substr($defaulttgbgcolour, 1);
            }
            $defaulttgbghvrcolour = get_config('format_collblct', 'defaulttogglebackgroundhovercolour');
            ...
            $courseformatoptionsedit = array(
                'numsections' => array(
                    'label' => new lang_string('numbersections', 'format_collblct'),
                    'element_type' => 'select',
                ...
                'displayinstructions' => array(
                    'label' => new lang_string('displayinstructions', 'format_collblct'),
            ...
            if (has_capability('format/collblct:changelayout', $context)) {
                $layoutelementvalues = $this->generate_default_entry(
                    ...
                    // In insertion order and not numeric for sorting purposes.
                     array(
                        ...
                        1 => new lang_string('setlayout_all', 'format_collblct'),
                        // Toggle word and toggle section x.
                        3 => new lang_string('setlayout_toggle_word_section_x', 'format_collblct'),
                        // Toggle word and section number.
                        2 => new lang_string('setlayout_toggle_word_section_number', 'format_collblct'),
                        // Toggle section x and section number.
                        5 => new lang_string('setlayout_toggle_section_x_section_number', 'format_collblct'),
                        // Toggle word.
                        4 => new lang_string('setlayout_toggle_word', 'format_collblct'),
                        // Toggle section x.
                        8 => new lang_string('setlayout_toggle_section_x', 'format_collblct'),
                        // Section number.
                        6 => new lang_string('setlayout_section_number', 'format_collblct'),
                        // No additions.
                        7 => new lang_string('setlayout_no_additions', 'format_collblct')
                    )
                );
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => new lang_string('setlayoutelements', 'format_collblct'),
                ...
                $layoutstructurevalues = $this->generate_default_entry(
                    ...
                    array(
                        // Topic.
                        1 => new lang_string('setlayoutstructuretopic', 'format_collblct'),
                        // Week.
                        2 => new lang_string('setlayoutstructureweek', 'format_collblct'),
                        // Current Week First.
                        3 => new lang_string('setlayoutstructurelatweekfirst', 'format_collblct'),
                        // Current Topic First.
                        4 => new lang_string('setlayoutstructurecurrenttopicfirst', 'format_collblct'),
                        // Day.
                        5 => new lang_string('setlayoutstructureday', 'format_collblct')
                    )
                );
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => new lang_string('setlayoutstructure', 'format_collblct'),
                ...
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => new lang_string('setlayoutcolumnorientation', 'format_collblct'),
                ...
                $layoutcolumnsvalues = $this->generate_default_entry(
                    ...
                    0,
                    array(
                        1 => new lang_string('one', 'format_collblct'),
                        2 => new lang_string('two', 'format_collblct'),
                        3 => new lang_string('three', 'format_collblct'),
                        4 => new lang_string('four', 'format_collblct')
                    )
                );
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => new lang_string('setlayoutcolumns', 'format_collblct'),
                ...
                $toggleiconpositionvalues = $this->generate_default_entry(
                    ...
                    0,
                    array(
                        1 => new lang_string('left', 'format_collblct'),
                        2 => new lang_string('right', 'format_collblct')
                    )
                );
                ...
                $courseformatoptionsedit['onesection'] = array(
                    'label' => new lang_string('onesection', 'format_collblct'),
                    'help' => 'onesection',
                    'help_component' => 'format_collblct',
                    'element_type' => 'select',
                ...
                $courseformatoptionsedit['showsectionsummary'] = array(
                    'label' => new lang_string('setshowsectionsummary', 'format_collblct'),
            ...
            if ((!empty($enabledplugins)) && (has_capability('format/collblct:changeactivitymeta', $context))) {
                ...
                $courseformatoptionsedit['showadditionalmoddata'] = array(
                    'label' => new lang_string('showadditionalmoddata', 'format_collblct', $stringenabled),
                ...
                if ((!empty($tcsettings['showadditionalmoddata'])) && ($tcsettings['showadditionalmoddata'] == 2)) {
                    $maxstudentsinfo = \format_collblct\activity::maxstudentsnotexceeded($courseid, true);
                    if ($maxstudentsinfo['maxstudents'] == 0) {
                        $activityinfostring = get_string('courseadditionalmoddatastudentsinfounlimited', 'format_collblct',
                            $maxstudentsinfo['nostudents']);
                    } else if (!$maxstudentsinfo['notexceeded']) {
                        $activityinfostring = get_string('courseadditionalmoddatastudentsinfolimitednoshow', 'format_collblct',
                            array('students' => $maxstudentsinfo['nostudents'], 'maxstudents' => $maxstudentsinfo['maxstudents']));
                    } else {
                        $activityinfostring = get_string('courseadditionalmoddatastudentsinfolimitedshow', 'format_collblct',
                            array('students' => $maxstudentsinfo['nostudents'], 'maxstudents' => $maxstudentsinfo['maxstudents']));
                    }

                    $courseformatoptionsedit['courseadditionalmoddatastudentsinfo'] = array(
                        'label' => get_string('courseadditionalmoddatastudentsinfo', 'format_collblct'),
                        'element_type' => 'static',
            ...
            if (has_capability('format/collblct:changetogglealignment', $context)) {
                $togglealignmentvalues = $this->generate_default_entry(
                    'togglealignment',
                    0,
                    array(
                        1 => new lang_string('left', 'format_collblct'),
                        2 => new lang_string('center', 'format_collblct'),
                        3 => new lang_string('right', 'format_collblct')
                    )
                );
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => new lang_string('settogglealignment', 'format_collblct'),
                ...
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => get_config('format_collblct', 'defaulttogglealignment'), 'element_type' => 'hidden');
            }
            if (has_capability('format/collblct:changetoggleiconset', $context)) {
                ...
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => new lang_string('settoggleiconset', 'format_collblct'),
                ...
                $courseformatoptionsedit['toggleiconfontclosed'] = array(
                    'label' => new lang_string('settoggleiconfontclosed', 'format_collblct'),
                    'help' => 'settoggleiconfontclosed',
                    'help_component' => 'format_collblct',
                    'element_type' => 'text'
                );
                $courseformatoptionsedit['toggleiconfontopen'] = array(
                    'label' => new lang_string('settoggleiconfontopen', 'format_collblct'),
                ...
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => new lang_string('settoggleallhover', 'format_collblct'),
            ...
            if (has_capability('format/collblct:changecolour', $context)) {
                $opacityvalues = array(
                    '-' => '',
                    '0.0' => '0.0',
                    '0.1' => '0.1',
                ...
                $courseformatoptionsedit['toggleforegroundcolour'] = array(
                    'label' => new lang_string('settoggleforegroundcolour', 'format_collblct'),
                ...
                $opacityvalues['-'] = new lang_string('default', 'format_collblct', get_config('format_collblct', 'defaulttoggleforegroundopacity'));
                $courseformatoptionsedit['toggleforegroundopacity'] = array(
                    'label' => new lang_string('settoggleforegroundopacity', 'format_collblct'),
                ...
                $courseformatoptionsedit['toggleforegroundhovercolour'] = array(
                    'label' => new lang_string('settoggleforegroundhovercolour', 'format_collblct'),
                ...
                $opacityvalues['-'] = new lang_string('default', 'format_collblct', get_config('format_collblct', 'defaulttoggleforegroundhoveropacity'));
                $courseformatoptionsedit['toggleforegroundhoveropacity'] = array(
                    'label' => new lang_string('settoggleforegroundhoveropacity', 'format_collblct'),
                ...
                $courseformatoptionsedit['togglebackgroundcolour'] = array(
                    'label' => new lang_string('settogglebackgroundcolour', 'format_collblct'),
                ...
                $opacityvalues['-'] = new lang_string('default', 'format_collblct', get_config('format_collblct', 'defaulttogglebackgroundopacity'));
                $courseformatoptionsedit['togglebackgroundopacity'] = array(
                    'label' => new lang_string('settogglebackgroundopacity', 'format_collblct'),
                ...
                $courseformatoptionsedit['togglebackgroundhovercolour'] = array(
                    'label' => new lang_string('settogglebackgroundhovercolour', 'format_collblct'),
                ...
                $opacityvalues['-'] = new lang_string('default', 'format_collblct', get_config('format_collblct', 'defaulttogglebackgroundhoveropacity'));
                $courseformatoptionsedit['togglebackgroundhoveropacity'] = array(
                    'label' => new lang_string('settogglebackgroundhoveropacity', 'format_collblct'),
            ...
            $courseformatoptionsedit['readme'] = array(
                    'label' => get_string('readme_title', 'format_collblct'),
                    'element_type' => 'static',
                    'element_attributes' => array(get_string('readme_desc', 'format_collblct', array('url' => $readme)))
                );
    ...
    private function generate_default_entry($settingname, $defaultindex, $values) {
        $defaultvalue = get_config('format_collblct', 'default'.$settingname);
        $defarray = array($defaultindex => new lang_string('default', 'format_collblct', $values[$defaultvalue]));
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        MoodleQuickForm::registerElementType(
            'tccolourpopup',
            "$CFG->dirroot/course/format/collblct/js/tc_colourpopup.php",
        ...
        $changelayout = has_capability('format/collblct:changelayout', $context);
        $changecolour = has_capability('format/collblct:changecolour', $context);
        $changetogglealignment = has_capability('format/collblct:changetogglealignment', $context);
        $changetoggleiconset = has_capability('format/collblct:changetoggleiconset', $context);
        $changeactivitymeta = has_capability('format/collblct:changeactivitymeta', $context);
        $resetall = is_siteadmin($USER); // Site admins only.

        $elements[] = $mform->addElement('header', 'ctreset', get_string('ctreset', 'format_collblct'));
        $mform->addHelpButton('ctreset', 'ctreset', 'format_collblct', '', true);

        $resetelements = array();
        $checkboxname = get_string('resetdisplayinstructions', 'format_collblct');
        $resetelements[] = & $mform->createElement('checkbox', 'resetdisplayinstructions', '', $checkboxname);
        $resetelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetdisplayinstructions', 'format_collblct'));

        if ($changelayout) {
            $checkboxname = get_string('resetlayout', 'format_collblct');
            $resetelements[] = & $mform->createElement('checkbox', 'resetlayout', '', $checkboxname);
            $resetelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetlayout', 'format_collblct'));
        }

        if ($changecolour) {
            $checkboxname = get_string('resetcolour', 'format_collblct');
            $resetelements[] = & $mform->createElement('checkbox', 'resetcolour', '', $checkboxname);
            $resetelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetcolour', 'format_collblct'));
        }

        if ($changetogglealignment) {
            $checkboxname = get_string('resettogglealignment', 'format_collblct');
            $resetelements[] = & $mform->createElement('checkbox', 'resettogglealignment', '', $checkboxname);
            $resetelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resettogglealignment', 'format_collblct'));
        }

        if ($changetoggleiconset) {
            $checkboxname = get_string('resettoggleiconset', 'format_collblct');
            $resetelements[] = & $mform->createElement('checkbox', 'resettoggleiconset', '', $checkboxname);
            $resetelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resettoggleiconset', 'format_collblct'));
        }

        if ($changeactivitymeta) {
            $checkboxname = get_string('resetactivitymeta', 'format_collblct');
            $resetelements[] = & $mform->createElement('checkbox', 'resetactivitymeta', '', $checkboxname);
            $resetelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetactivitymeta', 'format_collblct'));
        }

        $elements[] = $mform->addGroup($resetelements, 'resetgroup', get_string('resetgrp', 'format_collblct'), null, false);

        if ($resetall) {
            $resetallelements = array();

            $checkboxname = get_string('resetalldisplayinstructions', 'format_collblct');
            $resetallelements[] = & $mform->createElement('checkbox', 'resetalldisplayinstructions', '', $checkboxname);
            $resetallelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetalldisplayinstructions', 'format_collblct'));

            $checkboxname = get_string('resetalllayout', 'format_collblct');
            $resetallelements[] = & $mform->createElement('checkbox', 'resetalllayout', '', $checkboxname);
            $resetallelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetalllayout', 'format_collblct'));

            $checkboxname = get_string('resetallcolour', 'format_collblct');
            $resetallelements[] = & $mform->createElement('checkbox', 'resetallcolour', '', $checkboxname);
            $resetallelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetallcolour', 'format_collblct'));

            $checkboxname = get_string('resetalltogglealignment', 'format_collblct');
            $resetallelements[] = & $mform->createElement('checkbox', 'resetalltogglealignment', '', $checkboxname);
            $resetallelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetalltogglealignment', 'format_collblct'));

            $checkboxname = get_string('resetalltoggleiconset', 'format_collblct');
            $resetallelements[] = & $mform->createElement('checkbox', 'resetalltoggleiconset', '', $checkboxname);
            $resetallelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetalltoggleiconset', 'format_collblct'));

            $checkboxname = get_string('resetallactivitymeta', 'format_collblct');
            $resetallelements[] = & $mform->createElement('checkbox', 'resetallactivitymeta', '', $checkboxname);
            $resetallelements[] = & $mform->createElement('html', $OUTPUT->help_icon('resetallactivitymeta', 'format_collblct'));

            $elements[] = $mform->addGroup($resetallelements, 'resetallgroup',
                get_string('resetallgrp', 'format_collblct'), null, false);
        }
    ...
    public function edit_form_validation($data, $files, $errors) {
        ...
        if ($this->validate_colour($data['toggleforegroundcolour']) === false) {
            $retr['toggleforegroundcolour'] = get_string('colourrule', 'format_collblct');
        }
        if ($this->validate_colour($data['toggleforegroundhovercolour']) === false) {
            $retr['toggleforegroundhovercolour'] = get_string('colourrule', 'format_collblct');
        }
        if ($this->validate_colour($data['togglebackgroundcolour']) === false) {
            $retr['togglebackgroundcolour'] = get_string('colourrule', 'format_collblct');
        }
        if ($this->validate_colour($data['togglebackgroundhovercolour']) === false) {
            $retr['togglebackgroundhovercolour'] = get_string('colourrule', 'format_collblct');
        }
    ...
    public function update_course_format_options($data, $oldcourse = null) {
        ...
        if (($resetalldisplayinstructions) ||
            ...
            ($resetallactivitymeta)) {
                $this->reset_collblct_setting(0, $resetalldisplayinstructions, $resetalllayout, $resetallcolour,
                    $resetalltogglealignment, $resetalltoggleiconset, $resetallactivitymeta);
            $changes = true;
            ...
            ($resettoggleiconset) ||
            ($resetactivitymeta)) {
                $this->reset_collblct_setting($this->courseid, $resetdisplayinstructions, $resetlayout, $resetcolour,
                    $resettogglealignment, $resettoggleiconset, $resetactivitymeta);
            $changes = true;
        }
     ...
     */
    private function format_collblct_get_section_dates($section, $course) {
     ...
     */
    private function format_collblct_get_section_day($section, $course) {
        $onedayseconds = 86400;
        /* Hack alert. We add 2 hours to avoid possible DST problems. (e.g. we go into daylight
           savings and the date changes. */
        $startdate = $course->startdate + 7200;
     ...
     */
    public function reset_collblct_setting($courseid, $displayinstructions, $layout, $colour, $togglealignment, $toggleiconset,
        $activitymeta) {
        global $DB, $USER;
        ...
        if ($layout && has_capability('format/collblct:changelayout', $context) && $resetallifall) {
        ...
        if ($activitymeta && has_capability('format/collblct:changeactivitymeta', $context) && $resetallifall) {
            $updatedata['showadditionalmoddata'] = 0;
            $updateactivitymeta = true;
        }
        if ($togglealignment && has_capability('format/collblct:changetogglealignment', $context) && $resetallifall) {
            $updatedata['togglealignment'] = 0;
            $updatetogglealignment = true;
        }
        if ($colour && has_capability('format/collblct:changecolour', $context) && $resetallifall) {
        ...
        if ($toggleiconset && has_capability('format/collblct:changetoggleiconset', $context) && $resetallifall) {
     ...
     * in the old 'format_collblct_settings' table.
     * @param int $courseid If not 0, then a specific course to reset.
     ...
     */
    public function restore_collblct_setting($courseid, $layoutelement, $layoutstructure, $layoutcolumns, $tgfgcolour,
        ...
        $lco = get_config('format_collblct', 'defaultlayoutcolumnorientation');
        if (empty($lco)) {
     ...
     */
    public function update_collblct_columns_setting($layoutcolumns) {
        // Create data array.
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_collblct');
        }
        if (empty($editlabel)) {
            $course = $this->get_course();
            $title = $this->get_collblct_section_name($course, $section, false);
            $editlabel = new lang_string('newsectionname', 'format_collblct', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        if (($tcsettings['layoutstructure'] == 1) || ($tcsettings['layoutstructure'] == 4)) {
            if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
                // Format 'collblct' allows to set and remove markers in addition to common section actions.
        ...
        $renderer = $PAGE->get_renderer('format_collblct');
 ...
 * @return \core\output\inplace_editable
 */
function format_collblct_inplace_editable($itemtype, $itemid, $newvalue) {
    global $CFG;
    ...
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        ...
        $section = $DB->get_record_sql(
            ...
            array($itemid, 'collblct'), MUST_EXIST);
 ...
 */
function callback_collblct_definition() {
    return get_string('sectionname', 'format_collblct');
}
