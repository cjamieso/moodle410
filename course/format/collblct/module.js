...
M.format_collblct.thesparezeros = "00000000000000000000000000";
M.format_collblct.togglestate = false;
M.format_collblct.courseid = 0;
M.format_collblct.togglePersistence = 1; // Toggle persistence - 1 = on, 0 = off.
M.format_collblct.ourYUI = false;
M.format_collblct.numSections = 0;
M.format_collblct.oneTopic = false;
M.format_collblct.currentTopic = null; // For oneTopic when true represents the current open topic or null if none.
M.format_collblct.currentTopicNum = false; // For oneTopic when true represents the current open topic number or 0 if none.
M.format_collblct.userIsEditing = false;
 ...
 */
M.format_collblct.init = function(Y, theCourseId, theToggleState, theNumSections, theTogglePersistence,
    ...
    if (toggleHeight !== false) {
        ...
        var collblctShiftWindow = function() {
        ...
        window.addEventListener("hashchange", collblctShiftWindow);
    }
};
...
M.format_collblct.allOpenClick = function(e) {
    ...
    M.format_collblct.ourYUI.all(".toggle span.the_toggle").addClass('toggle_open').removeClass('toggle_closed')
...
M.format_collblct.allCloseClick = function(e) {
    ...
    M.format_collblct.ourYUI.all(".toggle span.the_toggle").addClass('toggle_closed').removeClass('toggle_open')
...
M.format_collblct.set_user_preference = function(name, value) {
    YUI().use('io', function(Y) {
        var url = M.cfg.wwwroot + '/course/format/collblct/setcollblctpref.php?sesskey=' + M.cfg.sesskey + '&pref=' + encodeURI(name) + '&value=' + encodeURI(value); // jshint ignore:line
        ...
        if (M.cfg.developerdebug) {
            cfg.on.failure = function() {
                console.log("Error updating collblct preference '" + name + "' using AJAX.  Almost certainly your session has timed out.  Clicking this link will repeat the AJAX call that failed so you can see the error: "); // jshint ignore:line
            };
        }
